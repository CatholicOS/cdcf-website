...
```bash
# Create a team member (linked to About page)
...
# Create an academic council member (linked to an academic collaboration post)
python scripts/cdcf_api.py create-team-member \
  --title "Dr. John Smith" \
  --content "<p>Professor of Computer Science.</p>" \
  --council academic_council \
  --collab-post-id 601 \
  --member-title "Department Chair"
...
# Create an academic collaboration
python scripts/cdcf_api.py create-academic-collaboration \
  --title "University Name" \
  --collab-description "Description of collaboration" \
  --collab-university "University Name" \
  --collab-department "Computer Science" \
  --collab-website-url "https://example.edu"
```
...
**Important:** The module is located at `scripts/cdcf_api.py`. When importing from inline Python or scripts outside the `scripts/` directory, you must add the path manually:
...
For scripts inside the `scripts/` directory, import directly:
...
# Create an academic council member linked to a collaboration post
result = client.create_team_member(
    title="Dr. John Smith",
    content="<p>Professor.</p>",
    council="academic_council",
    collab_post_id=601,
)
