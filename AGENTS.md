...
- **Audience verification first** (before any network round-trip). The token must be a JWT (configured via `OIDC_TOKEN_TYPE_JWT` in cdcf-infra) and its `aud` claim must include the CDCF Website Web app's client ID, configured via the `CDCF_ZITADEL_EXPECTED_AUD` constant in `wp-config.php`. Without this check, a token validly issued by Zitadel for a sibling property (LitCal/OntoKit/BibleGet) would otherwise pass the email-based WP user lookup, because umbrella login names are emails globally (`UserEmailAsUsername=true`). `CDCF_ZITADEL_EXPECTED_AUD` defaults to `''`, and an empty value fails every audience check closed — no Bearer token is accepted at all until the constant is set. The constant's value comes from the handoff produced by `cdcf-infra/auth/setup-zitadel.sh --provision-cdcf-website`.
