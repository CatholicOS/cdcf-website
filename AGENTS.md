...
| `POST`  | `/translate-all`             | Create-or-reuse drafts for ALL non-source Polylang languages of `source_id` and queue each translation atomically (one `pll_save_post_translations` call). Fixes the lost-update race that orphaned 2-3 of 5 langs per Translate-All. |
...
- **Userinfo round-trip** validates the token's signature, expiry, and revocation status against `/oidc/v1/userinfo`; accepts only when `email_verified=true` AND a `sub` claim is present. The `sub` claim is the immutable cross-system identity primary key — without it the validator refuses to resolve or auto-provision.
- **User resolution + auto-provisioning** (Phase 5, supersedes prior "no auto-provisioning" stance for the Subscriber path):
  1. Look up by Zitadel `sub` claim, stored as WP user meta `cdcf_zitadel_sub`. If found and the claim's email differs from `wp_users.user_email`, update the WP email so the two stay in sync — **never create a second WP user**. This is the email-drift sync that lets a Zitadel user change their email without losing their WP identity.
  2. Fallback: look up by email (one-time migration for WP users created before the sub binding existed). On match, write `cdcf_zitadel_sub` user-meta so future requests take the fast path.
  3. Auto-provision a Subscriber if both lookups miss. WP `user_login` = full email (immutable per WP, matches the umbrella Zitadel `UserEmailAsUsername=true` invariant at creation time); `user_pass` is randomly generated and never surfaced — sign-in must always flow through Zitadel. The Zitadel `name` claim seeds `display_name`, falling back to the email.
- **Transient cache** of accepted `(sha256(token) → user_id)` pairs for 60 seconds — raw tokens never land in the WP options table.
- **Fails closed on every unhappy path** (opaque non-JWT token, wrong audience, non-200 userinfo, malformed JSON, `email_verified !== true`, missing `sub` claim, auto-provision failure): returns the previous filter's `$user_id` unchanged so Application Passwords + auth cookies + other auth methods keep working untouched. "No WP user match" is **not** a fail-closed case anymore — see the user-resolution + auto-provisioning bullet above.
